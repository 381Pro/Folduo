...
[0.1.16をダウンロード](https://github.com/bunkaich/Folduo/releases/tag/v0.1.16)
...
リリースの `folduo-wallpaper-setup-0.1.16.zip` を展開し、PCにPython 3とAndroid SDK platform-tools（ADB）を用意します。USBデバッグを許可した端末を1台接続し、展開先で実行します。
...
1. リリースの `Folduo-0.1.16.apk` をインストールします。ADBなら `adb install -r Folduo-0.1.16.apk` でも導入できます。
...
配布物の `SHA256SUMS` と、macOSなら `shasum -a 256 Folduo-0.1.16.apk`、Linuxなら `sha256sum Folduo-0.1.16.apk` の結果を照合できます。
